...
test('shared player isolates content, local numbering and config under a deployment subpath', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    for (const [id, ratio] of [['one', '4/3'], ['two', '16/9']]) {
      mkdirSync(join(root, 'episodes', id), { recursive: true })
      const diagram = id === 'two' ? ['```mermaid', 'graph LR; A-->B', '```'].join('\n') : ''
...
`)
    ...
    await buildSharedPlayer(root, ['one', 'two'], output, '/preview/')
    const manifest = JSON.parse(readFileSync(join(output, 'player/manifest.json'), 'utf8'))
    assert.deepEqual(Object.keys(manifest.episodes), ['one', 'two'])
    ...
    writeFileSync(request, JSON.stringify({ ids: ['one', 'two'], output, base: '/preview/' }))
